...
typedef enum { STDWAVE,MC_STD,MC_MONO,MC_STEREO,MC_QUAD,MC_LCRS,MC_BFMT,MC_DOLBY_5_1,MC_WAVE_EX } psf_channelformat;
...
long psf_sndWriteShortFrames(int sfd, const short *buf, DWORD nFrames);

/* 	get current m/c frame position in file, or PSF_E_BADARG with bad sfd*/    
long psf_sndTell(int sfd);
...
int psf_sndSeek(int sfd,long offset,int seekmode);
...
/* currently supported: .wav, .aif, .aiff,.aifc,.afc */
...
unsigned int psf_sndGetDither(int sfd);
